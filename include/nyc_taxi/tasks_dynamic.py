...
def run_data_quality_checks(year_month, 
                            min_rows=10000, 
                            max_null_pct=1.0, 
                            max_neg_duration_pct=0.1):
    """
    Simple quality checks on raw Parquet taxi files in S3:
    - no null pickup/dropoff timestamps
    - non-negative trip duration (in seconds)
    ...
    for col, dtype in dtype_mapping.items():
        if col in df.columns:
            try:
                ...
                dtype_errors.append(f"Schema error: column '{col}' has dtype '{dtype}', expected '{dtype_mapping[col]}'")
    ...
    # check columns are in datetime format, non conforming entries set to na.
    ...
    # null value count across all required columns
    req_cols_null_count = len(df[df["tpep_pickup_datetime"].isna() | 
    ...
    req_cols_null_pct =  req_cols_null_count / file_row_count if file_row_count else 0
    neg_duration_pct = neg_duration_count / file_row_count if file_row_count else 0

    # raise error if thresholds exceeded
    if req_cols_null_pct > max_null_pct:
        raise errors.NullThresholdError(req_cols_null_count, max_null_pct)
